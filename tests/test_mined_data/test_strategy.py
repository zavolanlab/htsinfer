...
with open('tests/test_mined_data/mined_test_data.tsv',
    ...
    for index, row in source.iterrows():
        if row['layout'] == 'SE':
            sample_se = 'tests/test_mined_data/results_sra_downloads/' \
                        + row['sample'] + '/' + row['sample'] + '.fastq.gz'
            subprocess.Popen(HTS_CMD + ' ' + sample_se
                             + '> tests/test_mined_data/results_htsinfer/'
                             + row['sample'] + '_result.json',
                             shell=True, executable='/bin/bash').communicate()
        else:
            sample_1 = 'tests/test_mined_data/results_sra_downloads/' \
                       + row['sample'] + '/' + row['sample'] + '_1.fastq.gz'
            sample_2 = 'tests/test_mined_data/results_sra_downloads/' \
    ...
    for index, row in source.iterrows():
        if row['layout'] == 'SE':
            with open('tests/test_mined_data/results_htsinfer/'
                      + row['sample'] + '_result.json',
